...
/plugin install moirai@test-marketplace
```
...
| Skill | Purpose |
|-------|---------|
| `moirai-workflow` | Master workflow, agent roles, delegation patterns |
...
**Note:** Skills marked `user-invocable: true` can be invoked directly by users via `/skill-name` or the Skill tool. All Moirai skills are user-invocable for flexibility.

## Architecture: Skills-First

Moirai uses **skills instead of MCP tools**. This means:
...
Lachesis: I'll coordinate this with Moirai.

[Invokes context-template skill]
[Creates .moirai/sessions/ii-5092/]
...
Analyze `.moirai/sessions/*/review-*.md` over time to improve your process.
...
├── skills/
│   ├── moirai-workflow/      # Master workflow skill
