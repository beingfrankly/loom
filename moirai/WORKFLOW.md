# Moirai Plugin Workflow

This document explains how the Moirai plugin coordinates multi-agent development workflows.

## Overview

Moirai follows a **Context → Plan → Review → Execute** flow, with **Lachesis** (the coordinator agent) directing specialist agents through each phase.
...
Moirai uses **skills instead of MCP tools**. Before writing any artifact, agents must invoke the corresponding template skill:
...
sequenceDiagram
    ...
    Note over L,FS: .moirai/sessions/ii-5092/context.md
```
...
All artifacts are stored in `.moirai/sessions/{ticket-id}/`:
...
|-------|---------|---------|
| `moirai-workflow` | Lachesis, Planner, Code Reviewer | Master workflow and delegation patterns |
